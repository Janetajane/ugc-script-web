...
// SANGAT AMAN: Mengambil API Key dari brankas rahasia Cloudflare
...
export default function App() {
  ...
  // FUNGSI GENERATE SCRIPT VIA GEMINI API
  const handleGenerateScript = async () => {
    ...
    try {
      ...
      const promptInstruction = `
        Anda adalah copywriter konten TikTok/Shopee Affiliate profesional. 
        Tolong buatkan naskah/skrip Voice Over.

        INFORMASI:
        - Detail Tambahan: ${productText || 'Tolong analisis dari gambar terlampir untuk mengetahui apa produknya.'}
        - Gaya Bahasa: ${tone}
        - Perkiraan Durasi Teks: ${duration.words} kata.
        - Bahasa: Indonesia yang natural, gaul, dan relevan dengan tren saat ini.

        FORMAT YANG WAJIB DIIKUTI:
        [HOOK]
        (Tulis 1-2 kalimat pembuka yang sangat memancing rasa penasaran atau relate dengan masalah penonton di awal video)

        [ISI SKRIP]
        (Tulis isi skrip promosi yang mengalir natural, tanpa tanda baca aneh, karena teks ini akan dibaca oleh AI Voice Over di CapCut. Pastikan menyebutkan ajakan bertindak seperti 'cek keranjang kuning' di akhir)
      `;
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-indigo-100 overflow-hidden">
          ...
          <div className="p-4 md:p-6 bg-slate-50/50 border-b border-indigo-50 space-y-5">
                ...
                <div className="space-y-3">
                  <div>
                    <select 
                      ...
                      className="w-full p-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-xs md:text-sm bg-white cursor-pointer"
                    >
                  ...
                  <div>
                    <select 
                      value={duration.id}
                      onChange={(e) => setDuration(DURATIONS.find(d => d.id === e.target.value))}
                      className="w-full p-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-xs md:text-sm bg-white cursor-pointer"
                    >
            ...
            <button 
              ...
              className={\`w-full py-3.5 md:py-4 rounded-xl font-bold flex items-center justify-center gap-2 transition-all text-base md:text-lg mt-2 \${
                isGenerating || (!productText.trim() && !productImageBase64)
            ...
            >
              {isGenerating ? (
                ...
                <><Wand2 size={20} /> Tuliskan Hook & Skrip Saya!</>
              )}
            </button>
          </div>
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              {script && (
                <button 
                  ...
                  className={\`text-xs md:text-sm font-bold flex items-center gap-1.5 px-3 py-1.5 rounded-lg transition-colors shadow-sm border \${
                    isCopied 
